...
- Multiple audio format support (.mp3, .aac, .wav, .flac, .m4a, .opus)
...
def setup_logging(output_dir: str):
    ...
    log_file = os.path.join(
        output_dir, f"transcription_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    )
    logging.basicConfig(
        level=logging.INFO,
...
def worker_process(
    ...
    try:
        ...
        if not debug:
            ...
            if debug_log:
                debug_log.write(
                    f"Worker {worker_id} (GPU{gpu_id}): Debug mode - stderr NOT redirected\n"
                )
                debug_log.flush()
        ...
        if transcriber_type == "parakeet_tdt":
            ...
            log_queue.put(
                f"Worker {worker_id} (GPU{gpu_id}): ERROR - Unknown transcriber type: {transcriber_type}"
            )
        ...
        if not torch.cuda.is_available():
            log_queue.put(
                f"Worker {worker_id} (GPU{gpu_id}): ERROR - CUDA not available!"
            )
            ready_queue.put({"worker_id": worker_id, "status": "failed"})
            return
        ...
        log_queue.put(
            f"Worker {worker_id} (GPU{gpu_id}): Using device {actual_device} ({device_name}, {vram_total:.1f}GB VRAM)"
        )

        # Load model
        log_queue.put(
            f"Worker {worker_id} (GPU{gpu_id}): Loading {transcriber_type} model: {transcriber_model}"
        )
        ...
        try:
            ...
            if transcriber_type in ["parakeet_tdt", "parakeet"] and enable_cuda_graphs:
                ...
                try:
                    ...
                    try:
                        ...
                        log_queue.put(
                            f"Worker {worker_id} (GPU{gpu_id}): CUDA graphs enabled with cuda-python {cuda_version}"
                        )
                    except AttributeError:
                        log_queue.put(
                            f"Worker {worker_id} (GPU{gpu_id}): CUDA graphs enabled with cuda-python (version unknown)"
                        )
                except ImportError:
                    log_queue.put(
                        f"Worker {worker_id} (GPU{gpu_id}): WARNING - cuda-python not found, CUDA graphs may not work optimally"
                    )
            else:
            ...
            if transcriber_type == "parakeet_tdt":
                ...
                transcriber = CanaryTranscriber(
                    model_checkpoint=transcriber_model, torch_dtype="float16"
                )

            vram_used = torch.cuda.memory_allocated(actual_device) / 1e9
            log_queue.put(
                f"Worker {worker_id} (GPU{gpu_id}): Model loaded! (Using {vram_used:.1f}GB VRAM)"
            )
        ...
        while True:
            ...
            if batch_files is None:  # Shutdown signal
                log_queue.put(
                    f"Worker {worker_id} (GPU{gpu_id}): Received shutdown signal"
                )
                break
            ...
            try:
                ...
                try:
                    ...
                    log_queue.put(
                        f"Worker {worker_id} (GPU{gpu_id}): Audio loading failed, trying alternative method: {str(audio_error)}"
                    )
                    ...
                    for audio_path in files_to_process:
                        try:
                            ...
                            audio_array, sr = librosa.load(
                                audio_path, sr=16000, dtype=np.float32
                            )
                            audio_arrays.append(audio_array)
                        except Exception as e:
                            log_queue.put(
                                f"Worker {worker_id} (GPU{gpu_id}): Failed to load {audio_path}: {e}"
                            )
                    ...
                    valid_indices = [
                        i for i, arr in enumerate(audio_arrays) if len(arr) > 0
                    ]
        ...
        log_queue.put(
            f"Worker {worker_id} (GPU{gpu_id}): Completed {files_processed:,} files"
        )
...
def log_writer_process(log_queue: mp.Queue, log_file: str, debug: bool = False):
    ...
    try:
        ...
        with open(log_file, "a", encoding="utf-8") as f:
            startup_msg = (
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Log writer started\n"
            )
            f.write(startup_msg)
            f.flush()
...
Supported audio formats: .mp3, .aac, .wav, .flac, .m4a, .opus
Transcripts are saved as .txt files alongside the original audio files.
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=[".mp3", ".aac", ".wav", ".flac", ".m4a", ".opus"],
        help="Audio file extensions to process (default: .mp3 .aac .wav .flac .m4a .opus)",
    )
    ...
    if args.transcriber in ["parakeet_tdt", "parakeet"] and enable_cuda_graphs:
        ...
        try:
            ...
            print(
                f"   ⚠️  cuda-python not found: Install with 'pip install cuda-python>=12.3' for 10-30% speedup"
            )
    ...
    if args.debug:
        ...
        print(
            f"   - Log writer errors will be written to /tmp/transcriber_log_writer_crash.log"
        )
    ...
    try:
        log_writer = mp.Process(
            target=log_writer_process, args=(log_queue, log_file, args.debug)
        )
        log_writer.start()
        if args.debug:
    ...
    log_queue.put(
        f"Existing: {existing_transcripts:,}, To process: {files_to_process:,}"
    )
    ...
    for i in range(total_workers):
        ...
        end_idx = (
            start_idx + files_per_worker if i < total_workers - 1 else len(audio_files)
        )
    ...
    while workers_ready < total_workers and not worker_failed:
        try:
            ...
            if msg["status"] == "ready":
                workers_ready += 1
                print(
                    f"✅ Worker {msg['worker_id']} ready ({workers_ready}/{total_workers})"
                )
            else:
